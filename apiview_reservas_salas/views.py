...
def crear_reserva(request):
    ...
    if not data.get('fecha_inicio') or not data.get('fecha_fin') or not data.get('sala') or not data.get('usuario'):
        return JsonResponse({'error': 'Faltan campos requeridos'}, status=400)
    if not Sala.objects.filter(id=data['sala']).exists():
    ...
    if ReservaSala.objects.filter(sala_id=data['sala'], fecha_inicio__lt=data['fecha_fin'], fecha_fin__gt=data['fecha_inicio']).exists():
        return JsonResponse({'error': 'La sala ya está reservada para las fechas seleccionadas'}, status=400)
...
def obtener_reserva(request, reserva_id):
    try:
        reserva = ReservaSala.objects.get(id=reserva_id)
...
def actualizar_reserva(request, reserva_id):
    ...
    if data['fecha_inicio'] >= data['fecha_fin']:
        return JsonResponse({'error': 'La fecha de inicio debe ser anterior a la fecha de fin'}, status=400)
    if ReservaSala.objects.filter(sala_id=data['sala'], fecha_inicio__lt=data['fecha_fin'], fecha_fin__gt=data['fecha_inicio']).exclude(id=reserva_id).exists():
        return JsonResponse({'error': 'La sala ya está reservada para las fechas seleccionadas'}, status=400)
    
    reserva.fecha_inicio = data['fecha_inicio']
    reserva.fecha_fin = data['fecha_fin']
    reserva.sala_id = data['sala']
